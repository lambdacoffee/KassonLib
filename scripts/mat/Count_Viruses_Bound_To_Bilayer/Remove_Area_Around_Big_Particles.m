function [CurrentImage,BinaryCurrentImage] = Remove_Area_Around_Big_Particles(Options,...
    ...
    for j = 1:NumberOfVirusesFound
        if VirusProperties(j).Area > Options.MinAreaBig
            ...
            SizeOfSquareAroundCurrVirus = (sqrt(CurrentVirusArea)*2+20);

                %Coordinates are set up to define the area around the Virus
                %of interest (i.e. the ROI)  
                CurrentBoxLeft = max([round(CurrentVirusX) - round(SizeOfSquareAroundCurrVirus/2),...
                    1]);
                CurrentBoxRight = min([round(CurrentVirusX) + round(SizeOfSquareAroundCurrVirus/2),...
                    ImageWidth]);
                CurrentBoxTop = max([round(CurrentVirusY) - round(SizeOfSquareAroundCurrVirus/2),...
                    1]);
                CurrentBoxBottom = min([round(CurrentVirusY) + round(SizeOfSquareAroundCurrVirus/2),...
                    ImageHeight]);
