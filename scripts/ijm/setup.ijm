/*
* This is the setup script for the Kasson Tool package.
...
function handleImageJ(user_choice, kasson_lib_directory) {
	...
	*/
	...
	imagej_interface_path = kasson_lib_directory + "scripts" + File.separator + "ijm" + File.separator + "imagejinterface.ijm";
	imagej_interface_txt = File.openAsString(imagej_interface_path);
	macro_path = kasson_lib_directory + "scripts" + File.separator + "ijm" + File.separator + "Kasson_Tool.ijm";
	...
	message = "Installation complete - Kasson Tool will be available";
	message += "\non the following menu path of the ImageJ toolbar:";
	message += "\nMacros >>> Kasson Tool" + key_bind;
...
function setupBox() {
	...
	* 			for key binding
	*/
	title = "Startup Kasson Tool Installer";
	if (determineIfFiji()) {
		...
		platform = "ImageJ";
		key_bind = "L";
	} message = "Preparing to install Kasson Tool for " + platform + "...\n";
	message += "Bind 'Kasson Tool' to shortcut?\n";
	message += "This will allow hotkey access upon startup via the [" + key_bind + "] key.\n";
	message += "The Kasson Tool will still be accessible on the" + platform + " toolbar, located at:\n";
	message += "Plugins >>> {scroll all the way down} >>> Tools >>> Kasson Tool";
...
function main() {
	kasson_lib_dir = getArgument();
	if (lengthOf(kasson_lib_dir) == 0) {
		...
		File.close(txt_file);
	} status_filepath = kasson_lib_dir + "log" + File.separator + "status.txt";
	if (!getStatus(status_filepath)) {
		exit("Kasson Tool already installed!");
	} macro_tool_file_path = kasson_lib_dir + "scripts" + File.separator + "ijm" + File.separator + "Kasson_Tool.ijm";
	...
	if (determineIfFiji()) {
		...
		if (user_choice) {
			...
			run("Add Shortcut... ", "shortcut=" + key_bind + " command=[Kasson Tool]");
