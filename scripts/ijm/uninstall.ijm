...
function handleImageJ() {
	...
	imagej_interface_path = macrodir + "KassonLib" + File.separator + "scripts" + File.separator + "ijm" + File.separator + "imagejinterface.ijm";
	if (!File.exists(imagej_interface_path) {
		imagej_interface_path = macrodir + "KassonLib-master" + File.separator + "scripts" + File.separator + "ijm" + File.separator + "imagejinterface.ijm";
		if (!File.exists(imagej_interface_path) {
			exit("FATAL ERROR: CANNOT LOCATE KassonLib DIRECTORY!");
		}
	} imagej_interface_txt = File.openAsString(imagej_interface_path);
	startup_text_head = substring(startup_macros_txt, 0, indexOf(startup_macros_txt, "macro \"Kasson Tool"));
	startup_text_tail = substring(startup_macros_txt, indexOf(startup_macros_txt, "macro \"Kasson Tool"));
...
function getStatus(kasson_lib_dir) {
	...
	// exits if Kasson Tool is not installed so as not to throw error.
	...
	if (File.exists(status_filepath)) {
		...
		if (indexOf(status, "uninstalled") != -1)
			{exit("Kasson Tool is not installed!");}
...
function main() {
	...
	if (determineIfFiji()) {
		...
		tool_path = plugins + "Tools" + File.separator + "Kasson_Tool.ijm";
		platform = "Fiji";
		...
		run("Remove Shortcut...", "shortcut=[Kasson Tool]");
